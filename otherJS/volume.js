...
function vmGetCCSliderIn_volume() {
	...
	if (vmCachedCCSliderIn_volume) {
		vmCachedCCSliderIn_volume.style.transition = vmHeightTransitionMode_volume === "none" ? "none" : "height 0.1s ease"
	}
...
function vmSetScaleTransition_volume(durationSec, withOriginDelay) {
	...
	if (!target) return
	const base = target === volumeMain_volume ? vmBaseTransition_volume : vmBaseTransitionCC_volume || getComputedStyle(target).transition
...
function vmApplyScale_volume(raw, recoverWhenInside, dtSec) {
	...
	if (raw > 100) {
		...
		const extra2 = volumeCTN_volume.classList.contains("small") && !!controlCenter.classList.contains("open") ? extra * 10 : extra
	...
	if (recoverWhenInside && vmWasOvershoot_volume) {
		const recover = vmOvershootMax_volume <= 0 ? 0.3 : 0.3 + (Math.min(vmOvershootMax_volume, vmOvershootTime_volume) / vmOvershootMax_volume) * 0.3
...
function vmInit_volume() {
	...
	if (volumeDownBtn_volume) volumeDownBtn_volume.addEventListener("pointerdown", (e) => vmOnButtonDown_volume(-2.5, e))
