...

{
	...
	if (localStorage.getItem("version") !== vstt) {
		if (parseInt(localStorage.getItem("version")) <= 1000) {
...
{
	currentWallpaperOnStyle = allWallpaperOnStyle[parseInt(localStorage.getItem(`allWallpaperOnStyle-${deviceType}`)) || 1]
}
{
...
}
{
	...
	document.getElementById("phoneName").textContent = localStorage.getItem("phoneName") || buildProp[`device.${deviceType}.name`]
}
{
	// load dark mode
...
}
{
	...
	document.querySelector(`#app_SettingsAppSysNav [data-nav="${localStorage.getItem("nav") || "swipe"}"]`).classList.add("active")
}

// Finished loading
document.getElementById("passwordIn4").textContent = "password: " + (correctPassword !== "" ? correctPassword : "not set")
