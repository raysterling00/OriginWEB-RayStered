...
async function unlockAnimWA(currentAppScreenTmp = currentAppScreen) {
	...
	// favApp
	const delay = (groupUnlockAnim.length - 1) * delayForUnlockAnim - (groupUnlockAnim.length - 1) * delayEasingForUnlockAnim
	...
	lastUnlockAnim = document.getElementById("favApp").getAnimations()[document.getElementById("favApp").getAnimations().length - 1]
	if (lastUnlockAnim) {
...
{
	const activeItem = document.querySelector('#app_SettingsAppLockEditor [name="allWallpaperOnStyle"] .itemChild.active')
