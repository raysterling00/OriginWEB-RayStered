...
function pointerMovingIcon(e) {
	...
	if (tempSwapState) {
		const sameSlot = slot && tempSwapState.container === slot.container && tempSwapState.row === slot.row && tempSwapState.col === slot.col
		if (!sameSlot) {
			if (!pendingRestoreTimeout) {
...
async function pointerUpIconWhileDragIcon(e) {
	...
	if (dropSlot) {
		if (
			tempSwapState &&
			(tempSwapState.container !== dropSlot.container || tempSwapState.row !== dropSlot.row || tempSwapState.col !== dropSlot.col)
		) {
			restoreTempSwap()
...
function extendIcon(icon, rowSpan = 1, colSpan = 2) {
	...
	for (const [r, c] of candidates) {
		...
		if (isAreaFree(container, r, c, rowSpan, colSpan, icon)) {
			...
			icon.dataset.size = document.querySelector(".previewIcon").dataset.size = document.querySelector(`#${icon.dataset.app}`).dataset.size = sizeKey
...
async function addApps(
	...
	if (getAllIcons().length >= 80) {
		tb_system("maximum is 80 applications")
		return
	}
	...
	if (skipAutoPlace) {
		const fallbackScreen = currentAppScreen || document.getElementById("appScreen1") || (await createAppScreen("appScreen1"))
	...
	Object.entries(layout).forEach(async ([screenId, screenIcons]) => {
		...
		Object.values(screenIcons).forEach(async (item) => {
			...
			if (spanRow > 1 || spanCol > 1) {
				...
				icon.dataset.size = document.getElementById(item.id).dataset.size = `${item.rowEnd - item.rowStart}x${item.colEnd - item.colStart}`
			} else {
...
async function loadAppLayout() {
	...
	const state = JSON.parse(data)
	const { layout = {}, removedApps: savedRemoved = [], addedApps: savedAdded = [], screenCounter: savedCounter = 2 } = state
...
async function updateIconUnlockPosAnim(icon) {
	...
	const tt = icon.dataset.size === "1x2" ? "e" : icon.dataset.size === "2x1" ? "k" : icon.dataset.size === "2x2" ? "d" : "v"
	...
	return new Promise((resolve) => {
		requestAnimationFrame(() => {
			allIcons.forEach((icon) => {
				...
				upsertCssRule(sheet, `#${appId}`, `${posX}:${offsetX}px; bottom:${offsetY}px; width:${iconW}px; height:${iconH}px;`)

				upsertCssRule(sheet, `#${appId}.open`, `${posX}:0%; bottom:50%; translate:0 50%; height: 100%; width: 100%;`)

				const row = parseInt(icon.style.gridArea?.split("/")[0]) || parseInt(getComputedStyle(icon).gridRowStart)
				const col = parseInt(icon.style.gridArea?.split("/")[1]) || parseInt(getComputedStyle(icon).gridColumnStart)
