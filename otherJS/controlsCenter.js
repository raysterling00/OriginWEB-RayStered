...
function openControlsCenter() {
	...
	const token = ccAnimToken,
		anim = [{ transform: "scale(0.6) translateY(-20px)", opacity: 0 }, { opacity: 0 }, { transform: "scale(1)", opacity: 1 }],
...
async function closeControlsCenter(fromSystem = false) {
	if (!controlCenter.classList.contains("open")) return
	ccAnimToken += 1
	const anim = [{ transform: "scale(1)", opacity: 1 }, { opacity: 0.8 }, { opacity: 0 }, { transform: "scale(0.6) translateY(-60px)", opacity: 0 }],
		timing = { duration: 250, fill: "forwards", easing: "ease-out" }
	controlCenter.classList.remove("open")
	statusBarLeft.style.transition = statusBarRight.style.transition = "transform 0.3s, translate calc(0.3s * var(--bg-speedAnimation))"
...
function backAnim_gridcc() {
	...
	controlCenter.style.transition = ""
	controlCenter.style.backdropFilter = `blur(7.5px) brightness(0.5)`
}
function updateTransformS2(y) {
	y = Math.min(dampY_cc(y), 200)
	const x = y / 3.5
	statusBarLeft.style.transition = statusBarRight.style.transition = "transform 0.08s, translate calc(0.3s * var(--bg-speedAnimation))"
	...
	controlCenter.style.backdropFilter = `blur(${ccO * 7.5}px) brightness(${1 - 0.45 * ccO})`
}
/* ================= CORE ================= */
...
function endDrag_cc(slider) {
	...
	isDragging_cc = false
	if (isOpen_cc) {
		statusBarLeft.style.transition = statusBarRight.style.transition = "transform 0.3s, translate calc(0.1s * var(--bg-speedAnimation))"
...
function getCCItemId(el, fallbackIndex = 0) {
	...
	if (!id) {
		const rand = typeof crypto !== "undefined" && crypto.randomUUID ? crypto.randomUUID() : `${Date.now()}-${ccUidCounter++}`
		id = `cc-uid-${rand}`
	}
	el.dataset.ccId = id
	return id
...
function addStoreItemToCC(storeItem) {
	...
	const keySelector = name && size ? `:scope > .item[name="${CSS.escape(name)}"][data-size="${CSS.escape(size)}"]` : null
...
function refreshCCSliderRefs() {
	...
	sliderInBrightness_brightness = sliderBrightness_brightness ? sliderBrightness_brightness.querySelector(".sliderIN") : null
}
refreshCCSliderRefs()
...
function getCCGridRect(el) {
	...
	if (rowEndRaw && rowEndRaw.includes("span")) rowEnd = rowStart + (parseInt(rowEndRaw.replace(/[^\d]/g, ""), 10) || 1)
	if (colEndRaw && colEndRaw.includes("span")) colEnd = colStart + (parseInt(colEndRaw.replace(/[^\d]/g, ""), 10) || 1)
...
function ccIsAreaFree(row, col, rowSpan, colSpan, ignoreEl, items) {
	...
	for (const child of list) {
		...
		if (!(targetRowEnd <= rect.rowStart || row >= rect.rowEnd || targetColEnd <= rect.colStart || col >= rect.colEnd)) return false
	}
	return true
}
const isAeraFreeCC = (slot) => slot && ccIsAreaFree(slot.row, slot.col, ccDragRowSpan, ccDragColSpan, ccDragTarget, ccDragItems)
...
function ccTryTempSwap(row, col) {
	...
	for (const icon of allIcons) {
		...
		const intersects = !(rect.rowEnd <= row || rect.rowStart >= row + rowSpan || rect.colEnd <= col || rect.colStart >= col + colSpan)
		if (!intersects) continue
		const fullyInside = rect.rowStart >= row && rect.rowEnd <= row + rowSpan && rect.colStart >= col && rect.colEnd <= col + colSpan
...
function ccPointerUp(e) {
	...
	const r = ccPreviewDropPos.getBoundingClientRect(),
		...
		slotY = r.top + r.height / 2,
		slot = ccGetHoverSlot(slotX, slotY, ccDragRowSpan, ccDragColSpan)
	if (isAeraFreeCC(slot)) ccDragTarget.style.gridArea = `${slot.row} / ${slot.col} / ${slot.row + ccDragRowSpan} / ${slot.col + ccDragColSpan}`
	else {
		ccRestoreTempSwap()
