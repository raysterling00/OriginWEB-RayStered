...
function runOpenScript(appId) {
	removeScript(`./appData/${appId}/js/close/close.js`)
	runScript(`./appData/${appId}/js/open/open.js`)
}

function runCloseScript(appId) {
	removeScript(`./appData/${appId}/js/open/open.js`)
	runScript(`./appData/${appId}/js/close/close.js`)
}
...
function closeAppToCenterCore({ easing, afterFinish }) {
	...
	const anim = appEl.animate(
		[{ transform: getComputedStyle(appEl).transform }, { opacity: 1 }, { transform: "translateY(-150px) scale(0.01)", opacity: 0 }],
		{
...
function closeAppToLeft() {
	...
	const anim = appEl.animate(
		[{ transform: getComputedStyle(appEl).transform }, { opacity: 1 }, { transform: "translateX(-100%) scale(0.8)", opacity: 1 }],
		{
...
function isVisuallyInsidePhone(el) {
	...
	return !(e.left >= phoneRect.left && e.top >= phoneRect.top && e.right <= phoneRect.right && e.bottom <= phoneRect.bottom)
}
...
function updateTransform(y, x, d = "0.1") {
	...
	currentOpeningElApp.style.transform = `translateX(${x}px) translateY(${-clampedY}px) scale(${1 - clampedY / SCALE_DIVISOR})`
}
