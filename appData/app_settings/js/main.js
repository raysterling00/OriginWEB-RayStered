...
const functionWhenOpenAppInApp_settings = {
	...
	app_SettingsAppDisplayAndBrightness: function () {
		...
		toggleFullScreenMode._fullScreenHandler = function (e) {
			...
			if (this.classList.contains("active")) {
				...
				document.documentElement.style.setProperty("--bg-widthPhone", `${width}`)
				document.documentElement.style.setProperty("--bg-heightPhone", `${height}`)
				document.documentElement.style.setProperty("--bg-borderRadiusPhone", `calc(41px * (min(${width} / 330, ${height} / 717)))`)
				...
				document.getElementById("allBtnForDebug").style.display = document.getElementById("frame").style.display = "none"
				...
				document.documentElement.style.setProperty("--bg-borderRadiusPhone", buildProp[`${deviceType}.borderRadius`])
				if (currentOpeningElApp)
					closeAppToCenterWithScript(() => {
						updateAppPosNoRemove()
						phoneRect = phone.getBoundingClientRect()
				...
				document.getElementById("allBtnForDebug").style.display = document.getElementById("frame").style.display = ""
	...
	app_SettingsAppLockEditor: function () {
		...
		el._pointerHandlers.up = (e) => {
			...
			document.documentElement.style.setProperty("--bg-lockClockTranslate", `${el._currentX}px ${el._currentY}px`)
	...
	app_SettingsAppPhoneColor: function () {
		{
			...
			scrollItems.handler = function (e) {
				document.querySelectorAll("#app_SettingsAppPhoneColor .horizontalScroll .itemChild.active").forEach((el) => {
					el.classList.remove("active")
				})
				e.target.classList.add("active")
			...
			const activeItem = document.querySelector(
				`[data-colorphone='${localStorage.getItem("colorPhone") ? localStorage.getItem("colorPhone") : "rgb(221, 221, 221)"}']`
			)
	...
	app_SettingsAppAnimation: function () {
		...
		}
		{
			...
			el._removeHandler = (e) => {
				...
				if (unlockAnimation == "HyperOS") filterForUnlockAnim = `blur(${rootStyle.getPropertyValue("--bg-advancedBlur").toString()})`
			}
	...
	app_SettingsAppPasswordAndSecurity: function () {
...

		{
			...
			const selectFpAnim = document.querySelector("#app_SettingsAppPasswordAndSecurity .select[name='fingerprintAnimation']")
